...
import 'package:media_kit/src/player/native/core/initializer_ohos.dart';
...
class Initializer {
  ...
  Future<Pointer<generated.mpv_handle>> create(
    ...
    // HarmonyOS: Isolate 无法访问动态库，NativeCallable wakeup 回调也不工作
    // 使用 Timer 轮询方式替代
    if (Platform.operatingSystem == 'ohos') {
      return InitializerOhos(mpv).create(callback, options: options);
    }
  ...
  void dispose(Pointer<generated.mpv_handle> ctx) {
    // HarmonyOS: 使用 Timer 轮询方式
    if (Platform.operatingSystem == 'ohos') {
      InitializerOhos(mpv).dispose(ctx);
      return;
    }
